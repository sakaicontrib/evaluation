...
SakaiProject.TableRenderer = function(form_id) {
  this.param_form = jQuery('form#'+form_id);
...
SakaiProject.TableRenderer.prototype.addSakaiPager = function(divID, pagenum, pagesize) {
  var firstPageButton = jQuery('#'+divID+' > form:eq(0) > div:eq(0) > input:eq(0)');
  var prevPageButton = jQuery('#'+divID+' > form:eq(0) > div:eq(0) > input:eq(1)');
  var nextPageButton = jQuery('#'+divID+' > form:eq(0) > div:eq(0) > input:eq(2)');
  var lastPageButton = jQuery('#'+divID+' > form:eq(0) > div:eq(0) > input:eq(3)');
  var pageSizeSelect = jQuery('#'+divID+' > form:eq(0) > div:eq(0) > select:eq(0)');
  var thisObj = this;
...
SakaiProject.TableRenderer.prototype.addSortableHeader = function(thID, sortby, sortdir) {
  var thisObj = this;
  jQuery('#'+thID).find('a').bind('click', this, function(e) {
...
SakaiProject.TableRenderer.prototype.addSearchFilter = function(divID, startingtext) {
  var filterInput = jQuery('#'+divID+' > input:eq(0)');
  var findButton = jQuery('#'+divID+' > input:eq(1)');
  var clearButton = jQuery('#'+divID+' > input:eq(2)');
  var thisObj = this;
