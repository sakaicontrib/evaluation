...
(function($) {
	...
	var doGraph = function(el){
		...
		var legend = '';
		var prefix = arr.prefix;
		var postfix = arr.postfix;
		var space = arr.barSpace; //space between bars
		var legendWidth = arr.legend ? arr.legendWidth : 0; //width of legend box
		var fieldWidth = ($(el).width()-legendWidth)/data.length; //width of bar
		var totalHeight =  $(el).height(); //total height of graph box
		var leg = new Array(); //legends array
		
		//max value in data, I use this to calculate height of bar
		var maxVal = max(data);
		var colPosition = 0; // for printing colors on simple bar graph
		var unique = Math.floor((Math.random()*100000) + 1);

 		for(var val in data){
 			var value, color;
 		    var valueData = data[val][0];
 			...
 			if (lbl2 == undefined) lbl2 = '';
 			var lbl3tooltip = '';
 			if (lbl3 == undefined || lbl3 == null || lbl3 == '') {
 			   lbl3tooltip = '';
 			} else {
 			   lbl3tooltip = ' title="'+lbl3+'"';
 			}
 			if(arr.type == 'multi')
 		
 			var out  = "<div class='jqBarGraphField graphField"+el.id+"' "+lbl3tooltip+" id='graphField"+unique+"' style='position: absolute'>";
 			out += "<div class='jqBarGraphValue graphValue"+el.id+"' id='graphValue"+unique+"'>"+prefix+value+postfix+"</div>";
 			
 			out += "<div class='jqBarGraphBar graphBar"+el.id+"' id='graphFieldBar"+unique+"' style='background-color:"+color+";position: relative; overflow: hidden;'>"+lbl2+"</div>";

			// if there is no legend or exist legends display lbl at the bottom
 			if(!arr.legend || arr.legends) {
 				out += "<div class='jqBarGraphLabel graphLabel"+el.id+"' "+lbl3tooltip+" id='graphLabel"+unique+"'>"+lbl+"</div>";
 			}
 			out += "</div>";
			...
			$(el).append(out);
 			...
 			var fieldHeight = (totalHeightBar*value)/maxVal;	
 			...
 			if(valueData instanceof Array){
				...
				if(arr.type=="multi"){
 				...
 				for (i in valueData){
 					...
 					o = "<div class='jqBarGraphSubBars subBars"+el.id+"' style='height:"+heig+"px; background-color: "+arr.colors[i]+"; left:"+wid*i+"px; color:"+arr.showValuesColor+"; font-size:"+fs+"px' >"+sv+"</div>";
 		...
 		var legend = createLegend(leg, unique); // create legend from array
	...
	createLegend = function(legendArr, unique){
		var legend = '';
