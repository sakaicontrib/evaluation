...
public class BeEvaluatedBoxRenderer {
            ...
            for (EvalEvaluation eval : evalsForInstructor) {
                ...
                if (EvalConstants.EVALUATION_STATE_INQUEUE.equals(eval.getState())
                    ...
                    for (EvalGroup group : groups) {
                        ...
                        UIInternalLink.make(evalrow, "evalResponsesTitleLink_preview", title, new EvalViewParameters(
                                PreviewEvalProducer.VIEW_ID, eval.getId(), group.evalGroupId));// view
                                                                                               // params
                        ...
                        int responsesNeeded = evalBeanUtils.getResponsesNeededToViewForResponseRate(responsesCount,
                                enrollmentsCount);
                        String responseString = EvalUtils.makeResponseRateStringFromCounts(responsesCount,
                                enrollmentsCount);
                        if (EvalUtils.checkStateAfter(evalState, EvalConstants.EVALUATION_STATE_CLOSED, true)) {
                            if (responsesNeeded == 0) {
                                UIInternalLink.make(evalrow, "evalResponsesDisplayLink", UIMessage.make(
                                        "controlevaluations.eval.responses.inline", new Object[] { responseString }),
                                        new ReportParameters(ReportsViewingProducer.VIEW_ID, eval.getId(),
                                                new String[] { group.evalGroupId }));
                            } else {
                                UIOutput responseOutput = UIOutput
                                        .make(evalrow, "evalResponsesDisplay", responseString);
                                responseOutput.decorate(new UITooltipDecorator(UIMessage.make(
    ...
    protected void makeDateComponent(UIContainer evalrow, EvalEvaluation eval, String evalState, String evalDateLabel,
            String evalDateItem, String evalStatusItem) {
        ...
        if (EvalConstants.EVALUATION_STATE_INQUEUE.equals(evalState)) {
            ...
            if (responsesNeeded == 0) {
                ...
                UIMessage.make(evalrow, evalStatusItem, "summary.status." + evalState).decorate(
                        new UITooltipDecorator(UIMessage.make("controlevaluations.eval.report.awaiting.responses",
                                new Object[] { responsesNeeded })));
            }
        } else {
