...
/**
 * This page is for a user with take evaluation permission to fill and submit
 * the evaluation
 * 
 * @author Aaron Zeckoski (aaronz@vt.edu)
 */
public class TakeEvalProducer implements ViewComponentProducer,
		ViewParamsReporter, NavigationCaseReporter, ActionResultInterceptor {
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams,
		...
		if (evaluationId == null) {
			// redirect over to the main view maybe?? (not sure how to do this
			// in RSF)
			log.error("User (" + currentUserId
					+ ") cannot take evaluation, eval id is not set");
		...
		if (eval == null) {
			throw new IllegalArgumentException("Invalid evaluationId ("
					+ evaluationId + "), cannot load evaluation");
		}

		UIMessage.make(tofill, "eval-title-header",
				"takeeval.eval.title.header");
		UIOutput.make(tofill, "evalTitle", eval.getTitle());

		/*
		 * check the states of the evaluation first to give the user a tip that
		 * this eval is not takeable, also avoids wasting time checking
		 * permissions when the evaluation certainly is closed, also allows us
		 * to give the user a nice custom message
		 */
		String evalState = evaluationService.returnAndFixEvalState(eval, true); // make
																				// sure
																				// state
																				// is
																				// up
																				// to
																				// date
		if (EvalUtils.checkStateBefore(evalState,
				EvalConstants.EVALUATION_STATE_ACTIVE, false)) {
			String dueDate = "--------";
			if (eval.getDueDate() != null) {
				dueDate = df.format(eval.getDueDate());
			}
			UIMessage.make(tofill, "eval-cannot-take-message",
					"takeeval.eval.not.open", new String[] {
							df.format(eval.getStartDate()), dueDate });
			log.info("User (" + currentUserId
					+ ") cannot take evaluation yet, not open until: "
					+ eval.getStartDate());
		} else if (EvalUtils.checkStateAfter(evalState,
				EvalConstants.EVALUATION_STATE_CLOSED, true)) {
			UIMessage.make(tofill, "eval-cannot-take-message",
					"takeeval.eval.closed", new String[] { df.format(eval
							.getDueDate()) });
			log.info("User (" + currentUserId
					+ ") cannot take evaluation anymore, closed on: "
					+ eval.getDueDate());
		} else {
		...
		List<EvalGroup> validGroups = new ArrayList<EvalGroup>(); // stores
																	// EvalGroup
																	// objects
		if (canAccess) {
			...
			if (evalGroupId != null) {
				...
				if (evaluationService.canTakeEvaluation(currentUserId,
						evaluationId, evalGroupId)) {
					userCanAccess = true;
				}
			} else {
				// select the first eval group the current user can take
				// evaluation in,
				// also store the total number so we can give the user a list to
				// choose from if there are more than one
				Map<Long, List<EvalAssignGroup>> m = evaluationService
						.getAssignGroupsForEvals(new Long[] { evaluationId },
								true, null);
				if (commonLogic.isUserAdmin(currentUserId)) {
					...
					for (int i = 0; i < assignGroups.size(); i++) {
						...
						if (evalGroupId == null) {
							// set the evalGroupId to the first valid group if
							// unset
							evalGroupId = assignGroup.getEvalGroupId();
						}
						validGroups.add(commonLogic
								.makeEvalGroupObject(assignGroup
										.getEvalGroupId()));
					}
				} else {
					EvalGroup[] evalGroups;
					if (EvalConstants.EVALUATION_AUTHCONTROL_NONE.equals(eval
							.getAuthControl())) {
						// anonymous eval allows any group to be evaluated
						List<EvalAssignGroup> assignGroups = m
								.get(evaluationId);
						evalGroups = new EvalGroup[assignGroups.size()];
						for (int i = 0; i < assignGroups.size(); i++) {
							EvalAssignGroup assignGroup = assignGroups.get(i);
							evalGroups[i] = commonLogic
									.makeEvalGroupObject(assignGroup
											.getEvalGroupId());
						}
					} else {
						List<EvalAssignUser> userAssignments = evaluationService
								.getParticipantsForEval(evaluationId,
										currentUserId, null,
										EvalAssignUser.TYPE_EVALUATOR, null,
										null, null);
						Set<String> evalGroupIds = EvalUtils
								.getGroupIdsFromUserAssignments(userAssignments);
						List<EvalGroup> groups = EvalUtils
								.makeGroupsFromGroupsIds(evalGroupIds,
										commonLogic);
						evalGroups = EvalUtils.getGroupsInCommon(groups, m
								.get(evaluationId));
					}
					for (int i = 0; i < evalGroups.length; i++) {
						EvalGroup group = evalGroups[i];
						if (evaluationService.canTakeEvaluation(currentUserId,
								evaluationId, group.evalGroupId)) {
							if (evalGroupId == null) {
								// set the evalGroupId to the first valid group
								// if unset
								evalGroupId = group.evalGroupId;
								userCanAccess = true;
							}
							validGroups.add(commonLogic
									.makeEvalGroupObject(group.evalGroupId));
						}
					}
				}
			}
			...
			if (userCanAccess) {
				// check if we had a failure during a previous submit and get
				// the missingKeys out if that was the failure
				...
				if (responseId == null) {
					...
					if (response == null) {
						...
						// EVALSYS-360 because of a hibernate issue this will
						// not work, do a binding instead -AZ
						// responseId =
						// localResponsesLogic.createResponse(evaluationId,
						// currentUserId, evalGroupId);
					} else {
				...
				if (responseId != null) {
					// load up the previous responses for this user (no need to
					// attempt to load if the response is new, there will be no
					// answers yet)
					answerMap = localResponsesLogic
							.getAnswersMapByTempItemAndAssociated(responseId);
				}

				// show the switch group selection and form if there are other
				// valid groups for this user
				if (validGroups.size() > 1) {
					...
					UIBranchContainer showSwitchGroup = UIBranchContainer.make(
							tofill, "show-switch-group:");
					UIMessage.make(showSwitchGroup, "switch-group-header",
							"takeeval.switch.group.header");
					UIForm chooseGroupForm = UIForm.make(showSwitchGroup,
							"switch-group-form", new EvalViewParameters(
									TakeEvalProducer.VIEW_ID, evaluationId,
									responseId, evalGroupId));
					UISelect.make(chooseGroupForm, "switch-group-list", values,
							labels, "#{evalGroupId}");
					UIMessage.make(chooseGroupForm, "switch-group-button",
							"takeeval.switch.group.button");
				}

				// fill in group title
				EvalGroup evalGroup = commonLogic
						.makeEvalGroupObject(evalGroupId);
				UIBranchContainer groupTitle = UIBranchContainer.make(tofill,
						"show-group-title:");
				UIMessage.make(groupTitle, "group-title-header",
						"takeeval.group.title.header");
				UIOutput.make(groupTitle, "group-title", evalGroup.title);

				// show instructions if not null
				if (eval.getInstructions() != null
						&& !("".equals(eval.getInstructions()))) {
					UIBranchContainer instructions = UIBranchContainer.make(
							tofill, "show-eval-instructions:");
					UIMessage.make(instructions, "eval-instructions-header",
							"takeeval.instructions.header");
					UIVerbatim.make(instructions, "eval-instructions", eval
							.getInstructions());
				}

				// get the setting and make sure it cannot be null (fix for
				// http://www.caret.cam.ac.uk/jira/browse/CTL-531)
				...
				if (studentAllowedLeaveUnanswered == null) {
					studentAllowedLeaveUnanswered = eval
							.getBlankResponsesAllowed();
					if (studentAllowedLeaveUnanswered == null) {
				...
				if (studentAllowedLeaveUnanswered == false) {
					UIBranchContainer note = UIBranchContainer.make(tofill,
							"show-eval-note:");
					UIMessage.make(note, "eval-note-text",
							"takeeval.user.must.answer.all.note");
				}

				UIBranchContainer formBranch = UIBranchContainer.make(tofill,
						"form-branch:");
				UIForm form = UIForm.make(formBranch, "evaluationForm");

				// bind the evaluation and evalGroup to the ones in the take
				// eval bean
				String evalOTP = "evaluationBeanLocator.";
				form.parameters.add(new UIELBinding("#{takeEvalBean.eval}",
						new ELReference(evalOTP + eval.getId())));
				form.parameters.add(new UIELBinding(
						"#{takeEvalBean.evalGroupId}", evalGroupId));

				// BEGIN the complex task of rendering the evaluation items

				// make the TI data structure
				TemplateItemDataList tidl = new TemplateItemDataList(
						evaluationId, evalGroupId, evaluationService,
						authoringService, hierarchyLogic, null);
				Set<String> instructorIds = tidl
						.getAssociateIds(EvalConstants.ITEM_CATEGORY_INSTRUCTOR);
				Set<String> assistantIds = tidl
						.getAssociateIds(EvalConstants.ITEM_CATEGORY_ASSISTANT);
				...
				if (selectionsEnabled) {
					...
					EvalAssignGroup assignGroup = evaluationService
							.getAssignGroupByEvalAndGroupId(evaluationId,
									evalGroupId);
					...
					instructorSelectionOption = EvalUtils.getSelectionSetting(
							...
							assignGroup, null);
					selectorType.put(SELECT_KEY_INSTRUCTOR,
							instructorSelectionOption);
					...
					if (assistantsEnabled) {
						assistantSelectionOption = EvalUtils
								.getSelectionSetting(
										...
										assignGroup, null);
						selectorType.put(SELECT_KEY_ASSISTANT,
					...
					for (Iterator<String> selector = selectorType.keySet()
							.iterator(); selector.hasNext();) {
						// FIXME findbugs says that getting keys like this is
						// inefficient, use Map.Entry
						String selectKey = (String) selector.next();
						String selectValue = (String) selectorType
								.get(selectKey);
						String uiTag = "select-" + selectKey;
						String selectionOTP = "#{takeEvalBean.selection"
								+ selectKey + "Ids}";
						...
						// We render the selection controls if there are at
						// least two Instructors/TAs
						if (selectionsEnabled && selectUserIds.size() > 1) {
							if (selectValue
									.equals(EvalAssignGroup.SELECTION_OPTION_ALL)) {
								// nothing special to do in all case
							} else if (selectValue
									.equals(EvalAssignGroup.SELECTION_OPTION_MULTIPLE)) {
								UIBranchContainer showSwitchGroup = UIBranchContainer
										.make(formBranch, uiTag + "-multiple:");
								for (String userId : selectUserIds) {
									EvalUser user = commonLogic
											.getEvalUserById(userId);
									UIBranchContainer row = UIBranchContainer
											.make(showSwitchGroup, uiTag
													+ "-multiple-row:");
									UIOutput checkBranch = UIOutput.make(row,
											uiTag + "-multiple-label",
											user.displayName);
									UIBoundBoolean b = UIBoundBoolean.make(row,
											uiTag + "-multiple-box",
											Boolean.FALSE);
									// we have to force the id so the JS block
									// checking can work
									b.decorators = new DecoratorList(
											new UIIDStrategyDecorator(
													user.userId));
									// have to force the target id so that the
									// label for works
									UILabelTargetDecorator uild = new UILabelTargetDecorator(
											b);
									uild.targetFullID = user.userId;
									checkBranch.decorators = new DecoratorList(
											uild);
								}
								// form.addParameter(new
								// UIELBinding(selectionOTP, ""));
							} else if (selectValue
									.equals(EvalAssignGroup.SELECTION_OPTION_ONE)) {
								List<String> value = new ArrayList<String>();
								List<String> label = new ArrayList<String>();
								value.add("default");
								label.add("--- Select ---"); // This should get
																// the string
																// equivalent
																// of:"takeeval.selection.dropdown"in
																// the bundle
								for (EvalUser user : commonLogic
										.getEvalUsersByIds(selectUserIds
												.toArray(new String[selectUserIds
														.size()]))) {
									value.add(user.userId);
									label.add(user.displayName);
								}
								UIBranchContainer showSwitchGroup = UIBranchContainer
										.make(formBranch, uiTag + "-one:");
								UIOutput.make(showSwitchGroup, uiTag
										+ "-one-header");
								UISelect
										.make(showSwitchGroup, uiTag
												+ "-one-list", value
												.toArray(new String[value
														.size()]), label
												.toArray(new String[label
														.size()]), selectionOTP);
							} else {
				...
				for (TemplateItemGroup tig : tidl.getTemplateItemGroups()) {
					...
					for (HierarchyNodeGroup hng : tig.hierarchyNodeGroups) {
						// render a node title
						if (hng.node != null) {
							// Showing the section title is system configurable
							// via the administrate view
							Boolean showHierSectionTitle = (Boolean) evalSettings
									.get(EvalSettings.DISPLAY_HIERARCHY_HEADERS);
							if (showHierSectionTitle) {
								UIBranchContainer nodeTitleBranch = UIBranchContainer
										.make(categorySectionBranch,
												"itemrow:nodeSection");
								UIOutput.make(nodeTitleBranch, "nodeTitle",
										hng.node.title);
							}
						}

						List<DataTemplateItem> dtis = hng
								.getDataTemplateItems(false);
						for (int i = 0; i < dtis.size(); i++) {
							DataTemplateItem dti = dtis.get(i);
							UIBranchContainer nodeItemsBranch = UIBranchContainer
									.make(categorySectionBranch,
											"itemrow:templateItem");
							if (i % 2 == 1) {
								nodeItemsBranch.decorate(new UIStyleDecorator(
										"itemsListOddLine")); // must match the
																// existing CSS
																// class
							}
				...
				UICommand.make(form, "submitEvaluation", UIMessage
						.make("takeeval.submit.button"),
				...
				UIMessage.make(tofill, "eval-cannot-take-message",
						"takeeval.user.cannot.take", new String[] {
								current.displayName, current.email,
								current.username });
				log.info("User (" + currentUserId
						+ ") cannot take evaluation: " + eval.getId());
			}
		}
	}
	 ...
	 * 
	 * @param categorySectionBranch
	 *            the parent container
	 * @param associateType
	 *            Assistant or Instructor value
	 * @param associateId
	 *            userIds for Assistants
	 * @param instructorIds
	 *            userIds for Instructors
	 * @param selectionOption
	 *            Selection setting for this user
	 * @param savedSelections
	 */
	private void showHeaders(UIBranchContainer categorySectionBranch,
			String associateType, String associateId,
		...
		if (!EvalAssignGroup.SELECTION_OPTION_ALL.equals(selectionOption)
			...
			//we are handling a response hide fieldset unless selected user's Id is not found in Map
				int found = 0;
	 ...
	 *            the parent container
	 * @param form
	 *            the form this item will associate with
	 * @param dti
	 *            the wrapped template item we will render
	 */
	private void renderItemPrep(UIBranchContainer parent, UIForm form,
			DataTemplateItem dti, EvalEvaluation eval) {
		...
		if (!TemplateItemUtils.isAnswerable(templateItem)) {
			// nothing to bind for unanswerable items unless it is a block
			// parent
			if (dti.blockChildItems != null) {
				// Handle the BLOCK PARENT special case - block item being
				// rendered
				...
				for (int j = 0; j < childList.size(); j++) {
					...
					// set up OTP paths
					String[] childAnswerOTP = setupCurrentAnswerBindings(form,
							currChildItem, dti.associateId);
			...
			currentAnswerOTP = setupCurrentAnswerBindings(form, templateItem,
					dti.associateId);
			displayIncrement++;
		}
		...
		evalProps.put(ItemRenderer.EVAL_PROP_ANSWER_REQUIRED, answerRequired
				.toString());
		if (dti.renderInvalid) {
			evalProps.put(ItemRenderer.EVAL_PROP_RENDER_INVALID, Boolean.TRUE
					.toString());
		}

		// render the item
		itemRenderer.renderItem(parent, "renderedItem:", currentAnswerOTP,
				templateItem, displayNumber, false, evalProps);

		/*
		 * increment the item counters, if we displayed 1 item, increment by 1,
		 * if we displayed a block, renderedItem has been incremented for each
		 * child, increment displayNumber by the number of blockChildren, here
		 * we are simply adding the display increment to the overall number -AZ
		 */
	...
	/**
	 * Generates the correct OTP path for the current answer associated with
	 * this templateItem, also handles the binding of the item to answer and the
	 * associatedId
	 * 
	 * @param form
	 *            the form to bind this data into
	 * @param templateItem
	 *            the template item which the answer should associate with
	 * @param associatedId
	 *            the associated ID to bind this TI with
	 * @return an array of binding strings from the TI to the answer (first) and
	 *         NA (second) which will bind to the input elements
	 */
	private String[] setupCurrentAnswerBindings(UIForm form,
			EvalTemplateItem templateItem, String associatedId) {
		// the associated type is set only if the associatedId is set
		String associatedType = null;
		if (associatedId != null) {
			associatedType = templateItem.getCategory(); // type will match the
															// template item
															// category
		}
		...
		if (responseId == null) {
			...
			// throw new
			// IllegalStateException("There is no response, something has failed to load correctly for takeeval");
			// EVALSYS-360 - have to use this again and add a binding for start
			// time
			form.parameters.add(new UIELBinding("takeEvalBean.startDate",
					new Date()));

			currAnswerOTP = responseAnswersOTP
					+ ResponseAnswersBeanLocator.NEW_1 + "."
					+ ResponseAnswersBeanLocator.NEW_PREFIX + renderedItemCount
					+ ".";
			newAnswer = true;
		} else {
			// if the user has answered this question before, point at their
			// response
			String key = TemplateItemUtils.makeTemplateItemAnswerKey(
					templateItem.getId(), associatedType, associatedId);
			EvalAnswer currAnswer = answerMap.get(key);
			if (currAnswer == null) {
				// this is a new answer
				newAnswer = true;
				currAnswerOTP = responseAnswersOTP + responseId + "."
						+ ResponseAnswersBeanLocator.NEW_PREFIX
						+ (renderedItemCount) + ".";
			} else {
				// existing answer
				newAnswer = false;
		...
		if (newAnswer) {
			...
			form.parameters
					.add(new UIELBinding(currAnswerOTP + "templateItem",
							new ELReference("templateItemWBL."
									+ templateItem.getId())));

			// bind the item to the answer
			form.parameters
					.add(new UIELBinding(currAnswerOTP + "item",
							new ELReference("itemWBL."
									+ templateItem.getItem().getId())));

			// bind the associated id (current instructor id or environment) and
			// type to the current answer
			if (associatedId != null) {
				// only do the binding if this is not null, otherwise it will
				// bind in empty strings
				form.parameters.add(new UIELBinding(currAnswerOTP
		...
		// generate binding for the UI input element (UIInput, UISelect, etc.)
		// to the correct part of answer
	...
	public List<NavigationCase> reportNavigationCases() {
		...
		i.add(new NavigationCase("success", new SimpleViewParameters(
				SummaryProducer.VIEW_ID)));
		return i;
	}
	 ...
	 * 
	 * @see
	 * uk.org.ponder.rsf.flow.ActionResultInterceptor#interceptActionResult(
	 * uk.org.ponder.rsf.flow.ARIResult,
	 * uk.org.ponder.rsf.viewstate.ViewParameters, java.lang.Object)
	 */
	public void interceptActionResult(ARIResult result,
		...
		if (etvp.evalCategory != null) {
			result.resultingView = new EvalCategoryViewParameters(
					ShowEvalCategoryProducer.VIEW_ID, etvp.evalCategory);
		}
	}
