...
public class ModifyBlockProducer implements ViewComponentProducer,ViewParamsReporter,DynamicNavigationCaseReporter{
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams, ComponentChecker checker) {	
		...
		if(evParameters != null && evParameters.templateItemIds != null){
			...
			boolean modify = false;//this variable indicate if it is for modify existing Block		
			boolean validScaleIds = true;//this variable indicate if the passed Ids have the same scale			
			...
			if(!modify && validScaleIds){//creating new block with the same scale case
				boolean shift = false;
				//get the first Block ID if any, and shift it to the first of the templateItems array
				for(int i=0; i< templateItems.length; i++){
					if(templateItems[i].getBlockParent() != null && templateItems[i].getBlockParent().booleanValue()==true){
						if(i > 0){
			...
			if(! validScaleIds){
				...
				UIOutput.make(showError, "errorMsg",messageLocator.getMessage("modifyblock.error.message"));
				UIOutput.make(showError, "back-button", messageLocator.getMessage("modifyblock.back.button"));
				...
				if(modify){//for modify existing block item
					...
					for(int i= 0; i< childList.size();i++){
						...
						UIOutput.make(radiobranch, "childOrder",child.getDisplayOrder().toString());
						UIInput.make(radiobranch,"queText",null,child.getItem().getItemText() );	
					} 				
				}else{
					if(createFromBlock){//render the first block child , then others, possibly other block child
						//TODO: wait for Aaron' s logic layer method to get child TemplateItems providing parent ID
						...
						for(int i= 0;i< templateItems.length; i++){
							if(TemplateItemUtils.getTemplateItemType(templateItems[i]).equals(EvalConstants.ITEM_TYPE_BLOCK) ){
								...
								for(int k=0; k< childs.size();k++){
									...
									UIBranchContainer radiobranch = UIBranchContainer.make(form,"queRow:",Integer.toString(orderNo)); //$NON-NLS-1$
									UIOutput.make(radiobranch, "childOrder",Integer.toString(orderNo+1));
									UIInput.make(radiobranch,"queText",null,myChild.getItem().getItemText() );
									orderNo++;
								}
