...
/**
 * DAO regression tests for EvaluationLockDao.
 */
@Slf4j
public class EvaluationLockDaoTest extends AbstractEvaluationDaoTest {
    ...
    public void testLockScale() {
        ...
        Assert.assertTrue( lockDao.lockScale( scaleLocked, Boolean.FALSE ) );
        Assert.assertFalse(scaleLocked.getLocked() );
        // check that unlocking an unlocked scale is not a problem
        Assert.assertFalse( lockDao.lockScale( scaleLocked, Boolean.FALSE ) );

        // check that locked scale that is locked by an item cannot be unlocked
        EvalScale scale1 = (EvalScale) persistence.findById(EvalScale.class, etdl.scale1.getId());
        Assert.assertTrue(scale1.getLocked() );
        Assert.assertFalse( lockDao.lockScale( scale1, Boolean.FALSE ) );
        Assert.assertTrue(scale1.getLocked() );
        // check that locking a locked scale is not a problem
        Assert.assertFalse( lockDao.lockScale( scale1, Boolean.TRUE ) );
    ...
    /**
     * Test method for {@link org.sakaiproject.evaluation.dao.EvaluationLockDao#lockItem(org.sakaiproject.evaluation.model.EvalItem, java.lang.Boolean)}.
     */
    @Test
    public void testLockItem() {
        ...
        Assert.assertTrue( lockDao.lockItem( etdl.item7, Boolean.TRUE ) );
        ...
        Assert.assertFalse( lockDao.lockItem( itemLocked, Boolean.TRUE ) );
        Assert.assertTrue(itemLocked.getLocked() );

        // check that locked item gets unlocked (no scale, not used)
        Assert.assertTrue(itemLocked.getLocked() );
        Assert.assertTrue( lockDao.lockItem( itemLocked, Boolean.FALSE ) );
        ...
        Assert.assertFalse( lockDao.lockItem( etdl.item1, Boolean.FALSE ) );
        Assert.assertTrue( etdl.item1.getLocked() );

        // check that locked item that is locked by a template can be locked without exception
        Assert.assertTrue( etdl.item1.getLocked() );
        Assert.assertFalse( lockDao.lockItem( etdl.item1, Boolean.TRUE ) );
        Assert.assertTrue( etdl.item1.getLocked() );
        ...
        Assert.assertTrue( lockDao.lockItem( itemUnlocked, Boolean.TRUE ) );
        ...
        Assert.assertTrue( itemUnlocked.getLocked() );
        Assert.assertTrue( lockDao.lockItem( itemUnlocked, Boolean.FALSE ) );
        Assert.assertFalse( itemUnlocked.getLocked() );
        ...
        Assert.assertTrue( lockDao.lockItem( etdl.item4, Boolean.TRUE ) );
        Assert.assertTrue( etdl.item4.getLocked() );

        Assert.assertTrue( lockDao.lockItem( etdl.item4, Boolean.FALSE ) );
    ...
    /**
     * Test method for {@link org.sakaiproject.evaluation.dao.EvaluationLockDao#lockTemplate(org.sakaiproject.evaluation.model.EvalTemplate, java.lang.Boolean)}.
     */
    @Test
    public void testLockTemplate() {
        ...
        Assert.assertTrue( lockDao.lockTemplate( etdl.templateAdminNoItems, Boolean.TRUE ) );
        ...
        Assert.assertTrue( etdl.templateAdminNoItems.getLocked() );
        Assert.assertFalse( lockDao.lockTemplate( etdl.templateAdminNoItems, Boolean.TRUE ) );
        Assert.assertTrue( etdl.templateAdminNoItems.getLocked() );

        // check that locked template gets unlocked (no items)
        Assert.assertTrue( etdl.templateAdminNoItems.getLocked() );
        Assert.assertTrue( lockDao.lockTemplate( etdl.templateAdminNoItems, Boolean.FALSE ) );
        Assert.assertFalse( etdl.templateAdminNoItems.getLocked() );
        ...
        Assert.assertFalse( lockDao.lockTemplate( etdl.templateUser, Boolean.FALSE ) );
        Assert.assertTrue( etdl.templateUser.getLocked() );

        // check that locked template that is locked by an evaluation can be locked without exception
        Assert.assertTrue( etdl.templateUser.getLocked() );
        Assert.assertFalse( lockDao.lockTemplate( etdl.templateUser, Boolean.TRUE ) );
        Assert.assertTrue( etdl.templateUser.getLocked() );
        ...
        Assert.assertTrue( lockDao.lockTemplate( etdl.templateUserUnused, Boolean.TRUE ) );
        ...
        Assert.assertTrue( etdl.templateUserUnused.getLocked() );
        Assert.assertTrue( lockDao.lockTemplate( etdl.templateUserUnused, Boolean.FALSE ) );
        Assert.assertFalse( etdl.templateUserUnused.getLocked() );
        ...
        Assert.assertTrue( lockDao.lockTemplate( etdl.templateUnused, Boolean.TRUE ) );
        ...
        Assert.assertTrue( etdl.templateUnused.getLocked() );
        Assert.assertTrue( lockDao.lockTemplate( etdl.templateUnused, Boolean.FALSE ) );
        Assert.assertFalse( etdl.templateUnused.getLocked() );
    ...
    /**
     * Test method for {@link org.sakaiproject.evaluation.dao.EvaluationLockDao#lockEvaluation(org.sakaiproject.evaluation.model.EvalEvaluation)}.
     */
    @Test
    public void testLockEvaluation() {
        ...
        Assert.assertTrue( lockDao.lockEvaluation( evalUnLocked, true ) );
        ...
        Assert.assertTrue( evalUnLocked.getLocked() );
        Assert.assertTrue( lockDao.lockEvaluation( evalUnLocked, false ) );
        Assert.assertFalse( evalUnLocked.getLocked() );
    ...
    /**
     * Test method for {@link org.sakaiproject.evaluation.dao.EvaluationLockDao#isUsedScale(java.lang.Long)}.
     */
    @Test
    public void testIsUsedScale() {
        Assert.assertTrue( lockDao.isUsedScale( etdl.scale1.getId() ) );
        Assert.assertTrue( lockDao.isUsedScale( etdl.scale2.getId() ) );
        Assert.assertFalse( lockDao.isUsedScale( etdl.scale3.getId() ) );
        Assert.assertFalse( lockDao.isUsedScale( etdl.scale4.getId() ) );
    }

    /**
     * Test method for {@link org.sakaiproject.evaluation.dao.EvaluationLockDao#isUsedItem(java.lang.Long)}.
     */
    @Test
    public void testIsUsedItem() {
        Assert.assertTrue( lockDao.isUsedItem( etdl.item1.getId() ) );
        Assert.assertTrue( lockDao.isUsedItem( etdl.item2.getId() ) );
        Assert.assertTrue( lockDao.isUsedItem( etdl.item3.getId() ) );
        Assert.assertFalse( lockDao.isUsedItem( etdl.item4.getId() ) );
        Assert.assertTrue( lockDao.isUsedItem( etdl.item5.getId() ) );
        Assert.assertTrue( lockDao.isUsedItem( etdl.item6.getId() ) );
        Assert.assertFalse( lockDao.isUsedItem( etdl.item7.getId() ) );
        Assert.assertFalse( lockDao.isUsedItem( etdl.item8.getId() ) );
        Assert.assertTrue( lockDao.isUsedItem( etdl.item9.getId() ) );
        Assert.assertTrue( lockDao.isUsedItem( etdl.item10.getId() ) );
    }

    /**
     * Test method for {@link org.sakaiproject.evaluation.dao.EvaluationLockDao#isUsedTemplate(java.lang.Long)}.
     */
    @Test
    public void testIsUsedTemplate() {
        Assert.assertTrue( lockDao.isUsedTemplate( etdl.templateAdmin.getId() ) );
        Assert.assertFalse( lockDao.isUsedTemplate( etdl.templateAdminBlock.getId() ) );
        Assert.assertFalse( lockDao.isUsedTemplate( etdl.templateAdminComplex.getId() ) );
        Assert.assertFalse( lockDao.isUsedTemplate( etdl.templateAdminNoItems.getId() ) );
        Assert.assertTrue( lockDao.isUsedTemplate( etdl.templatePublic.getId() ) );
        Assert.assertTrue( lockDao.isUsedTemplate( etdl.templatePublicUnused.getId() ) ); // used in this file
        Assert.assertFalse( lockDao.isUsedTemplate( etdl.templateUnused.getId() ) );
        Assert.assertTrue( lockDao.isUsedTemplate( etdl.templateUser.getId() ) );
        Assert.assertFalse( lockDao.isUsedTemplate( etdl.templateUserUnused.getId() ) );
    }

    @Test
    public void testObtainLock() {
        // check I can get a lock
        Assert.assertTrue( lockDao.obtainLock("AZ.my.lock", "AZ1", 100) );

        // check someone else cannot get my lock
        Assert.assertFalse( lockDao.obtainLock("AZ.my.lock", "AZ2", 100) );
        ...
        Assert.assertTrue( lockDao.obtainLock("AZ.my.lock", "AZ2", 100) );

        // check invalid arguments cause Assert.failure
        try {
            lockDao.obtainLock("AZ.my.lock", null, 1000);
            Assert.fail("Should have thrown an exception");
        } catch (IllegalArgumentException e) {
            Assert.assertNotNull(e);
        }
        try {
            lockDao.obtainLock(null, "AZ1", 1000);
            Assert.fail("Should have thrown an exception");
        } catch (IllegalArgumentException e) {
            Assert.assertNotNull(e);
        }
    ...
    public void testReleaseLock() {
        ...
        // check I can get a lock
        Assert.assertTrue( lockDao.obtainLock("AZ.R.lock", "AZ1", 1000) );

        // check someone else cannot get my lock
        Assert.assertFalse( lockDao.obtainLock("AZ.R.lock", "AZ2", 1000) );

        // check I can release my lock
        Assert.assertTrue( lockDao.releaseLock("AZ.R.lock", "AZ1") );

        // check someone else can get my lock now
        Assert.assertTrue( lockDao.obtainLock("AZ.R.lock", "AZ2", 1000) );

        // check I cannot get the lock anymore
        Assert.assertFalse( lockDao.obtainLock("AZ.R.lock", "AZ1", 1000) );

        // check they can release it
        Assert.assertTrue( lockDao.releaseLock("AZ.R.lock", "AZ2") );

        // check invalid arguments cause Assert.failure
        try {
            lockDao.releaseLock("AZ.R.lock", null);
            Assert.fail("Should have thrown an exception");
        } catch (IllegalArgumentException e) {
            Assert.assertNotNull(e);
        }
        try {
            lockDao.releaseLock(null, "AZ1");
            Assert.fail("Should have thrown an exception");
        } catch (IllegalArgumentException e) {
            Assert.assertNotNull(e);
        }
    ...
    public void testGetConsolidatedEmailMapping() {
        ...
        int count = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(true, (Date) null, false, (Date) null, EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_AVAILABLE);
        Assert.assertEquals(1, count);
        int deletions = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        Assert.assertEquals(1, deletions);
        count = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(true, new Date(), false, (Date) null, EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_AVAILABLE);
        Assert.assertEquals(1, count);
        deletions = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        Assert.assertEquals(1, deletions);

        // there should be two new evals ready to send announcements to, selected because the value of availableEmailSent is null
        int count1 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(true, (Date) null, false, (Date) null, EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_AVAILABLE);
        Assert.assertEquals(1,count1);
        List<Map<String,Object>> mapping1 = this.consolidatedEmailDao.getConsolidatedEmailMapping(true, 100, 0);
        Assert.assertNotNull(mapping1);
        Assert.assertEquals(1, mapping1.size());
        int deletions1 = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        Assert.assertEquals(1, deletions1);

        // Since those announcements have been sent, there should be none yet to be sent
        int count2 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(true, (Date) null, false, (Date) null, EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_AVAILABLE);
        Assert.assertEquals(0,count2);
        List<Map<String,Object>> mapping2 = this.consolidatedEmailDao.getConsolidatedEmailMapping(true, 100, 0);
        Assert.assertNotNull(mapping2);
        Assert.assertEquals(0, mapping2.size());
        int deletions2 = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        Assert.assertEquals(0, deletions2);

        // if we search for notices to be sent and ignore the date, we should find them again
        int count3 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(false, (Date) null, false, (Date) null, EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_AVAILABLE);
        Assert.assertEquals(1,count3);
        List<Map<String,Object>> mapping3 = this.consolidatedEmailDao.getConsolidatedEmailMapping(true, 100, 0);
        Assert.assertNotNull(mapping3);
        Assert.assertEquals(1, mapping3.size());
        int deletions3 = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        ...
        int count4 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(true, new Date(), true, new Date(), EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_REMINDER);
        Assert.assertEquals(0,count4);
        List<Map<String,Object>> mapping4 = this.consolidatedEmailDao.getConsolidatedEmailMapping(false, 100, 0);
        Assert.assertNotNull(mapping4);
        Assert.assertEquals(0, mapping4.size());
        int deletions4 = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        ...
        int count5 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(false, (Date) null, true, new Date(), EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_REMINDER);
        Assert.assertEquals(1,count5);
        List<Map<String,Object>> mapping5 = this.consolidatedEmailDao.getConsolidatedEmailMapping(false, 100, 0);
        Assert.assertNotNull(mapping5);
        Assert.assertEquals(1, mapping5.size());
        int deletions5 = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        Assert.assertEquals(1, deletions5);

        // if we do the same search again, we find 0 because they have just been sent 
        int count6 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(false, (Date) null, true, new Date(), EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_REMINDER);
        Assert.assertEquals(0,count6);
        List<Map<String,Object>> mapping6 = this.consolidatedEmailDao.getConsolidatedEmailMapping(false, 100, 0);
        Assert.assertNotNull(mapping6);
        Assert.assertEquals(0, mapping6.size());
        int deletions6 = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        Assert.assertEquals(0, deletions6);

        // if we search for evals needing reminders as if it were tomorrow, we should find 1
        int count7 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(false, (Date) null, true, new Date(System.currentTimeMillis() + MILLISECONDS_PER_DAY), EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_REMINDER);
        Assert.assertEquals(1,count7);
        List<Map<String,Object>> mapping7 = this.consolidatedEmailDao.getConsolidatedEmailMapping(false, 100, 0);
        Assert.assertNotNull(mapping7);
        Assert.assertEquals(1, mapping7.size());
        int deletions7 = this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        Assert.assertEquals(1, deletions7);

        // if we search for evals needing reminders as if it were tomorrow, we should find 1
        int count8 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(false, (Date) null, true, new Date(System.currentTimeMillis() + MILLISECONDS_PER_DAY), EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_REMINDER);
        Assert.assertEquals(1,count8);
        List<EvalEmailProcessingData> list = persistence.findAll(EvalEmailProcessingData.class);
        ...
        this.consolidatedEmailDao.resetConsolidatedEmailRecipients();
        EvalEmailProcessingData eepd = list.get(0);
        // update EvalAssignUser with completed time
        EvalEvaluation evaluation = persistence.findById(EvalEvaluation.class, eepd.getEvalId());
        List<EvalAssignUser> eaus = persistence.findAll(EvalAssignUser.class);
        Assert.assertNotNull(eaus);
        List<EvalAssignUser> matchingAssignUsers = assignmentDao.getParticipantsForEval(evaluation.getId(), eepd.getUserId(),
        ...
        persistence.update(eau);
        EvalAssignUser eau0 = persistence.findById(EvalAssignUser.class, eau.getId());
        Assert.assertNotNull(eau0);
        Assert.assertNotNull(eau0.getCompletedDate());
        int count9 = this.consolidatedEmailDao.selectConsolidatedEmailRecipients(false, (Date) null, true, new Date(System.currentTimeMillis() + MILLISECONDS_PER_DAY), EvalConstants.EMAIL_TEMPLATE_CONSOLIDATED_REMINDER);
        Assert.assertEquals(0,count9);
        List<Map<String,Object>> mapping9 = this.consolidatedEmailDao.getConsolidatedEmailMapping(false, 100, 0);
    ...
    public void testGetResponsesSavedInProgress() {
        ...
        this.persistence.save(responseActive);    	
        ...
        this.persistence.save(responseInactive);

        List<EvalResponse> evalOpenResponses = this.responseDao.getResponsesSavedInProgress(true);
        ...
        List<EvalResponse> evalClosedResponses = this.responseDao.getResponsesSavedInProgress(false);
